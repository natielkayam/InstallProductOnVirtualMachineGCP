...
$parameters = @{
    ...
    RemotePowershellUsername    = 'XXXXX'
...
$msg = "[run-setup-personal-lab.ps1] [$exactTime] Start running UI Automation tests"
Write-Output $msg
Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"

$msg = "[run-setup-personal-lab.ps1] [$($($(Get-Date).Subtract($startRunning)))] Log file for this run: $globalDeploymentLogFile"
Write-Output $msg
Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"

$currentRunningUser = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name
$msg = "[run-setup-personal-lab.ps1] [$($($(Get-Date).Subtract($startRunning)))] User that running this script is: $currentRunningUser"
Write-Output $msg
Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"
...
#     $msg = "[run-setup-personal-lab.ps1] [$($($(Get-Date).Subtract($startRunning)))] Warning: All Powershell based processes killed"
#     Write-Output $msg
#     Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"
# }
...
$msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($startRunning))] Folder where all TRX and logs will be saved is: $globalOutputTrxFolder"
Write-Output $msg
Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"
...
$msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($startRunning))] After sleep"
Write-Output $msg
Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"
...
foreach ($labName in $LABS.Keys) {
    ...
    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($startRunning))] Current lab is: $labName with settings $($lab | Out-String)"
    Write-Output $msg
    Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"
    ...
    $runningInstallations += Start-Job -Name $labName -Verbose -ErrorAction Continue -ScriptBlock {
        ...
        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Start working on the lab: $currentLabName"
        Write-Output "$msg`n"
        ...
        try {
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Connected successfully to vCenter Server at $ConnectedServer with user: $($using:CONFIGURATION.VMWareManagementUsername) and password"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg

        } catch {
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Exception occurred while trying to connect to VMWare Management server at: $($using:CONFIGURATION.VMWareManagementAddress) `nAdditional details:`n $( $Error[0].ToString() )"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        ...
        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] For lab: $currentLabName received a folder with name: $($folder.Name) and parent: $($folder.Parent.Name)"
        Write-Output "$msg`n"
        Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        ...
        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Received a list of  $($vmMachines.Count) VM's in folder: $($currentLabName): $($vmMachines.Name)"
        Write-Output "$msg`n"
        Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
...
#         }
        ...
        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Stopping VM's"
        # Write-Output "$msg`n"
        Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        ...
        foreach ($item in $vmWithSnapshots) {
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] VM: $($item.VM.Name) reverted to snapshot: $($item.Snapshot.Name)"
            # Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        }
        ...
        foreach ($item in $vmWithSnapshots) {
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Starting VM: $($item.VM.Name)"
            # Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            while ( $($startingVM.ExtensionData.guest.guestOperationsReady) -eq $false){
                ...
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Waiting for VM: $($startingVM.Name) to become available. Current state is: $($startingVM.ExtensionData.guest.guestState) and readyness is: $($startingVM.ExtensionData.guest.guestOperationsReady)"
                # Write-Output "$msg`n"
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
                ...
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] VM: $($startingVM.Name) state is: $($startingVM.ExtensionData.guest.guestState) and readyness is: $($startingVM.ExtensionData.guest.guestOperationsReady)"
                # Write-Output "$msg`n"
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            }
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] VM: $($item.VM.Name) re-started"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        }
        ...
        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Disconnected from $($using:CONFIGURATION.VMWareManagementAddress) server"
        Write-Output "$msg`n"
        Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
...
#             }
            ...
            # Diagnostic message
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Started to run installation tests on CCM --------"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            $performanceCCM = Measure-Command {
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Running script: $ccmInstallScript  with parameters"
                Write-Output $msg
                ...
                $runningScript = Start-Process -PassThru -NoNewWindow -FilePath 'pwsh.exe'                                          `
                                                -ArgumentList   "-NoLogo -NonInteractive -ExecutionPolicy Unrestricted ",           `
                                                                ...
                                                                "-RemoteComputerAddress     $($currentLab.CCM.Host)",               `
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Process with ID: $($runningScript.Id) and name: $($runningScript.Name) was started and saving output to: $logFileName"
                Write-Output "$msg`n"
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
                ...
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Process with ID: $($runningScript.Id) and name: $($runningScript.Name) has finished"
                ...
                if ((Test-Path $outputTrxFile -PathType Leaf) -eq $true) {
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [CCM] Display TRX of $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [CCM] TRX: Running tests outcome: $( $elem.Attributes[0].Value )"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [CCM] LAB installation outcome: $( $elem.Attributes[0].Value ) so installation error occurred"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [CCM] TRX: Additional data:"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [CCM] TRX file could not be found at: $outputTrxFile"
                    Write-Output "$msg`n" | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [CCM] TRX file is not found ! So LAB installation outcome: 'Failed' so installation error occurred"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                # Diagnostic message
                $msg = "`n[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Error is: $(Get-Content -Raw -Path $errorLogFileName)"
                Write-Output $msg | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
            }

            # Diagnostic message
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Deployment of CCM machine from $currentLabName took $($performanceCCM.ToString()) minutes"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
...
#             }
            ...
            # Diagnostic message
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] -------------------------------------------------"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg

            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Started to run installation tests on Store ------"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            $performanceStore = Measure-Command {
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Running script: $storeInstallScript  with parameters"
                Write-Output $msg  | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                $runningScript = Start-Process -PassThru -NoNewWindow -FilePath 'pwsh.exe'                                    `
                                                -ArgumentList   "-NoLogo -NonInteractive -ExecutionPolicy Unrestricted ",           `
                                                                ...
                                                                "-RemoteComputerAddress     $($currentLab.Store.Host)",             `
                                                                "-RemoteComputerUsername    $using:RemotePowershellUsername",       `
                                                                "-RemoteComputerPass        $using:RemotePowershellPassword",       `
                                                                "-GlobalConfiguration       $ConfigurationBase64String",            `
                                                                "-UiAutomationReleaseName   $using:UiAutomationReleaseName",        `
                ...
                if ((Test-Path $outputTrxFile -PathType Leaf) -eq $true) {
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE] Display TRX of $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE] TRX: Running tests outcome: $( $elem.Attributes[0].Value )"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE] LAB installation outcome: $( $elem.Attributes[0].Value ) so installation error occurred"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE] TRX: Additional data:"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] TRX file could not be found at: $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                # Diagnostic message
                $msg = "`n[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Error is: $(Get-Content -Raw -Path $errorLogFileName) `n"
                Write-Output $msg | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
            }

            # Diagnostic message
            $msg =  "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Deployment of STORE machine from $currentLabName took $($performanceStore.ToString()) time`n"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        }
...
#             }
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Started to run installation tests on Store 1 ----"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            $performanceStore = Measure-Command {
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Running script: $store1InstallScript  with parameters"
                Write-Output $msg | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                $runningScript = Start-Process -PassThru -NoNewWindow -FilePath 'pwsh.exe'                                          `
                                                -ArgumentList   "-NoLogo -NonInteractive -ExecutionPolicy Unrestricted ",           `
                                                                ...
                                                                "-RemoteComputerAddress     $($currentLab.Store1.Host)",            `
                                                                "-RemoteComputerUsername    $using:RemotePowershellUsername",       `
                                                                "-RemoteComputerPass        $using:RemotePowershellPassword",       `
                                                                "-GlobalConfiguration       $ConfigurationBase64String",            `
                                                                "-UiAutomationReleaseName   $using:UiAutomationReleaseName",        `
                ...
                if ((Test-Path $outputTrxFile -PathType Leaf) -eq $true) {
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE1] Display TRX of $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE1] TRX: Running tests outcome: $( $elem.Attributes[0].Value )"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE1] LAB installation outcome: $( $elem.Attributes[0].Value ) so installation error occurred"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE1] TRX: Additional data:"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
...
#             }
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Started to run installation tests on Store 2 ----"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            $performanceStore = Measure-Command {
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Running script: $store2InstallScript  with parameters"
                Write-Output $msg | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                $runningScript = Start-Process -PassThru -NoNewWindow -FilePath 'pwsh.exe'                                          `
                                                -ArgumentList   "-NoLogo -NonInteractive -ExecutionPolicy Unrestricted ",           `
                                                                ...
                                                                "-RemoteComputerAddress     $($currentLab.Store2.Host)",            `
                                                                "-RemoteComputerUsername    $using:RemotePowershellUsername",       `
                                                                "-RemoteComputerPass        $using:RemotePowershellPassword",       `
                                                                "-GlobalConfiguration       $ConfigurationBase64String",            `
                                                                "-UiAutomationReleaseName   $using:UiAutomationReleaseName",        `
                ...
                if ((Test-Path $outputTrxFile -PathType Leaf) -eq $true) {
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE2] Display TRX of $outputTrxFile" | Out-Default
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE2] TRX: Running tests outcome: $( $elem.Attributes[0].Value )"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE2] LAB installation outcome: $( $elem.Attributes[0].Value ) so installation error occurred"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE2] TRX: Additional data:"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [STORE2] TRX file could not be found at: $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
            ...
            # Diagnostic message
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Deployment of STORE machine from $currentLabName took $($performanceStore.ToString()) minutes"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        }
...
#             }
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Started to run installation tests on POS3 -------"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            $performancePOS3 = Measure-Command {
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Running script: $pos3InstallScript  with parameters"
                Write-Output $msg | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                $runningScript = Start-Process -PassThru -NoNewWindow -FilePath 'pwsh.exe'                                    `
                                                -ArgumentList   "-NoLogo -NonInteractive -ExecutionPolicy Unrestricted ",           `
                                                                ...
                                                                "-RemoteComputerAddress     $($currentLab.POS3.Host)",              `
                                                                "-RemoteComputerUsername    $using:RemotePowershellUsername",       `
                                                                "-RemoteComputerPass        $using:RemotePowershellPassword",       `
                                                                "-GlobalConfiguration       $ConfigurationBase64String",            `
                                                                "-UiAutomationReleaseName   $using:UiAutomationReleaseName",        `
                ...
                if ((Test-Path $outputTrxFile -PathType Leaf) -eq $true) {
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS3] Display TRX of $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS3] TRX: Running tests outcome: $( $elem.Attributes[0].Value )"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS3] TRX: Additional data:"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS3] TRX file could not be found at: $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                }
            ...
            # Diagnostic message
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Deployment of POS3 machine from $currentLabName took $($performancePOS3.ToString()) time`n"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        }
...
#             }
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Started to run installation tests on POS2 -------"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            $performancePOS2 = Measure-Command {
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Running script: $pos2InstallScript  with parameters"
                Write-Output $msg | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                $runningScript = Start-Process -PassThru -NoNewWindow -FilePath 'pwsh.exe'                                    `
                                                -ArgumentList   "-NoLogo -NonInteractive -ExecutionPolicy Unrestricted ",           `
                                                                ...
                                                                "-RemoteComputerAddress     $($currentLab.POS2.Host)",              `
                                                                "-RemoteComputerUsername    $using:RemotePowershellUsername",       `
                                                                "-RemoteComputerPass        $using:RemotePowershellPassword",       `
                                                                "-GlobalConfiguration       $ConfigurationBase64String",            `
                                                                "-UiAutomationReleaseName   $using:UiAutomationReleaseName",        `
                ...
                if ((Test-Path $outputTrxFile -PathType Leaf) -eq $true) {
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS2] Display TRX of $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS2] TRX: Running tests outcome: $( $elem.Attributes[0].Value )"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS2] TRX: Additional data:"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS2] TRX file could not be found at: $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                }
            ...
            # Diagnostic message
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Deployment of POS2 machine from $currentLabName took $($performancePOS2.TotalMinutes) time`n"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        }
...
#             }
            ...
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Started to run installation tests on POS1 -------"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
            ...
            $performancePOS1 = Measure-Command {
                ...
                # Diagnostic message
                $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Running script: $pos1InstallScript  with parameters"
                Write-Output $msg | Out-Default
                Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                ...
                $runningScript = Start-Process -PassThru -NoNewWindow -FilePath 'pwsh.exe'                                    `
                                                -ArgumentList   "-NoLogo -NonInteractive -ExecutionPolicy Unrestricted ",           `
                                                                ...
                                                                "-RemoteComputerAddress     $($currentLab.POS1.Host)",              `
                                                                "-RemoteComputerUsername    $using:RemotePowershellUsername",       `
                                                                "-RemoteComputerPass        $using:RemotePowershellPassword",       `
                                                                "-GlobalConfiguration       $ConfigurationBase64String",            `
                                                                "-UiAutomationReleaseName   $using:UiAutomationReleaseName",        `
                ...
                if ((Test-Path $outputTrxFile -PathType Leaf) -eq $true) {
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS1] Display TRX of $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS1] TRX: Running tests outcome: $( $elem.Attributes[0].Value )"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"

                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS1] TRX: Additional data:"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                    ...
                    $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] [POS1] TRX file could not be found at: $outputTrxFile"
                    Write-Output $msg | Out-Default
                    Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject "$msg`n"
                }
            ...
            # Diagnostic message
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Deployment of POS1 machine from $currentLabName took $($performancePOS1.ToString()) time`n"
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
        }
        ...
        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Finished to run installation test on the lab: $currentLabName"
        Write-Output "$msg`n"
        Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg

        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($using:startRunning))] [$currentLabName] Output log files and TRX saved to: $using:globalOutputTrxFolder"
        Write-Output "$msg`n"
        Out-File -Append -Encoding utf8 -FilePath $using:globalDeploymentLogFile -InputObject $msg
    }

    # Diagnostic message
    $msg = "[run-setup-personal-lab.ps1] [$labName] [$($(Get-Date).Subtract($startRunning))] Started to run installation test on the lab: $labName"
    Write-Output $msg
    Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"
}
...
while ($stillRunning) {
    ...
    if (($null -ne $jobsThatRunning) -and ($jobsThatRunning.Count -gt 0)) {
        ...
        $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($startRunning))] Receive a list of $($jobsThatRunning.Count) jobs that is still in state 'Running' -------"
        Write-Output "$msg`n"
        Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject $msg
        ...
        $jobsThatRunning | ForEach-Object {
            # Write-Output  $PSItem
            $msg = "[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($startRunning))] Job: $($PSItem.Name) -----------------------------------------------"
            Write-Output "$msg"
            Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject "$msg`n"
        ...
        if (($null -ne $otherJobs) -and ($otherJobs.Count -gt 0)) {
            $msg = "`n[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($startRunning))] List of jobs that in other state: Finished, Failed, Suspended, etc..."
            Write-Output "$msg`n"
            Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject $msg
...
$msg = "`n`n[run-setup-personal-lab.ps1] [$($(Get-Date).Subtract($startRunning))] Done running. All VM's are deployed. Quit now"
Write-Output "$msg`n"
Out-File -Append -Encoding utf8 -FilePath $globalDeploymentLogFile -InputObject $msg
